...
public class Q60060 {
  ...
  public int[] solution(String[] words, String[] queries) {
    ...
    for (String word : words) {
      trie.add(word,0);
      reversed.add(new StringBuffer(word).reverse().toString(), 0);
    }

    return Arrays.stream(queries)
                  .mapToInt(q -> count(q, trie, reversed))
                  .toArray();
  }
  ...
  private int count(String query, Node trie, Node reversedTrie) {
    if (query.startsWith("?")) {
      return reversedTrie.count(new StringBuffer(query).reverse().toString(), 0);
    }
